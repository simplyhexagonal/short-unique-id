...
export default class ShortUniqueId {
	...
	protected _normalizeDictionary = (
		...
		if (dictionary && Array.isArray(dictionary) && dictionary.length > 1) {
			...
			for (const [rangeTypeKey, rangeTypeValue] of Object.entries(ranges)) {
				this.dictRange = rangeTypeValue;

				this.lowerBound = this.dictRange[0];
				this.upperBound = this.dictRange[1];
				for (let i = this.lowerBound; this.lowerBound <= this.upperBound ? i < this.upperBound : i > this.upperBound;) {
					finalDict.push(String.fromCharCode(i));
					this.dictIndex = i;
					i = this.lowerBound <= this.upperBound ? i + 1 : i - 1;
				}
			}
		}
	...
	rnd = (
		uuidLength: number = this.uuidLength || DEFAULT_UUID_LENGTH,
	): string => {
	...
	randomUUID = (
		...
		for (j = 0; j < uuidLength; j += 1) {
			randomPartIdx =
				Number.parseInt((Math.random() * this.dictLength).toFixed(0), 10) %
				this.dictLength;
	...
	availableUUIDs = (uuidLength: number = this.uuidLength): number => {
		return Number.parseFloat(
			([...new Set(this.dict)].length ** uuidLength).toFixed(0),
		);
	};
	...
	approxMaxBeforeCollision = (
		...
		return Number.parseFloat(Math.sqrt((Math.PI / 2) * rounds).toFixed(20));
	};
	...
	parseStamp = (suid: string, format?: string): Date => {
		...
		return new Date(
			Number.parseInt(stamp.substring(rndIdx, rndIdx + 8), 16) * 1000,
		);
	};
	...
	validate = (
		...
		const finalDictionary = dictionary
			? this._normalizeDictionary(dictionary)
			: this.dict;
