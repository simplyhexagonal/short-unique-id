...
test({
	...
	fn(): void {
		...
		console.log(result);
		assert(
			/^[0123456789abcdef][0123456789abcdef][0123456789abcdef]$/.test(result),
		);
	},
});
...
test({
	...
	fn(): void {
		...
		/* tslint:disable no-magic-numbers */
		const nowTenStamp =
			Number.parseInt(Math.floor(+new Date() / 1000).toString(16), 16) * 1000;
		...
		const nowOmniStamp =
			Number.parseInt(Math.floor(+pastDate / 1000).toString(16), 16) * 1000;
...
test({
	...
	fn(): void {
		...
		let testRegex = /[0-9abcdef]{11}[0-3]-a-[ab]{2}/;
...
test({
	...
	fn(): void {
		...
		const timestamp = new Date("2023-01-29T03:21:21.000Z");
		const nowStamp =
			Number.parseInt(Math.floor(+timestamp / 1000).toString(16), 16) * 1000;
